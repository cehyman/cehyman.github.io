...
const dest = i => ({ x: 0, y: i * -4, scale: 1, rot: -10 + Math.random() * 20, delay: i * 100 })
...
function Deck() {
  ...
  const [props, set] = useSprings(cards.length, i => ({ ...dest(i), from: from(i) })) // Create a bunch of springs using the helpers above
  ...
  const bind = useDrag(({ args: [index], down, delta: [xDelta], distance, direction: [xDir], velocity }) => {
    ...
    if (!down && gone.size === cards.length) setTimeout(() => gone.clear() || set(i => dest(i)), 600)
  })
